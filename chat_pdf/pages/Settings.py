...
from configs import FILES_FOLDER, MODEL_OPTIONS, SEARCH_TYPES, get_config
...
def settings_page():
    ...
    model_name = st.selectbox(label='Modify model', options=MODEL_OPTIONS, index=MODEL_OPTIONS.index(get_config('model_name')))
    retrieval_search_type = st.selectbox(label='Modify search type', options=SEARCH_TYPES,index=SEARCH_TYPES.index(get_config('retrieval_search_type')))
    retrieval_args = st.text_input('Modify search args', value=json.dumps(get_config('retrieval_args')))
    prompt = st.text_area('Modify prompt', height=350, value=get_config('prompt'), disabled=True)
